...
class DefenderCore {
  ...
  async init({ authToken, propertiesPath, session, api }) {
    ...
    let sitename = propValue(props, 'server.sitename')
    let exchangeType = propValue(props, 'trading.exchange')
    let tradingKey = propValue(props, 'trading_api_key')
      || propValue(props, 'trading_api_Key')
      || propValue(props, 'default_api_key')
    let tradingSecret = propValue(props, 'trading_api_secret')
      || propValue(props, 'trading_api_Secret')
      || propValue(props, 'default_api_secret')
    let hostname = propValue(props, 'server.address') || propValue(props, 'server.hostname', 'localhost')
    let contextPath = propValue(props, 'server.context_path', '/')
    let port = propValue(props, 'server.port')
    let proto = propValue(props, 'server.ssl.key-store').length ? 'https' : 'http'
    let url = `${proto}://${hostname}:${port}${contextPath}`
