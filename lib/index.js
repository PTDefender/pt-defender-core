...
class DefenderCore {
   ...
   * @return {Promise}
   */
  async init({ authToken, properties, propertiesPath, session, api }) {
    if (!authToken) throw new Error('authToken is required')
    if (!propertiesPath && !properties) throw new Error('propertiesPath or properties is required')
    ...
    if (propertiesPath) {
      ...
      for (const key of Object.keys(properties)) { props.set(key, properties[key]) }
    }

    let serverToken = propValue(props, 'server.api_token')

    if (!serverToken || !serverToken.length) {
      serverToken = uuid().replace(/[^a-z0-9]/g, '')
    ...
    const tradingKey = propValue(props, 'trading_api_key')
      || propValue(props, 'trading_api_Key')
      || propValue(props, 'default_api_key')
    const tradingSecret = propValue(props, 'trading_api_secret')
    ...
    const url = `${proto}://${hostname}:${port}${contextPath}`

    this._api = new Api({ authToken, exchangeType, tradingKey, tradingSecret, session, api })
    this._pt = new ProfitTrailer({ license, url, serverToken, sitename })
