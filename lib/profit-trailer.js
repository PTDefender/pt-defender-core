...
class ProfitTrailer {
   ...
   */
  constructor({ baseURL, token, sitename = null }) {
   ...
   * @return {Promise}
   */
  async setHotConfig(text) {
    for (const line of text.split('\n')) {
      const [propertyName, value] = line.split('=').map(str => str.trim())
      await this._post('/api/v2/hotconfig/save', { propertyName, value })
    }
  }
