...
const { BittrexClient: Bittrex } = require('bittrex-typescript')
...
class BittrexClient {
  ...
  async accountValue({ currency }) {
    ...
    return {
      ...
      free: result.Available ? result.Available.toNumber() : 0,
      locked: result.Pending ? result.Pending.toNumber() : 0
    }
  }
  ...
  async openOrder({ currency, baseCurrency, side, quantity, price }) {
    ...
    switch (side) {
    case 'BUY':
      result = await this._api.buyLimit(symbol, adjustedQuantity, adjustedPrice)
      break
    case 'SELL':
      result = await this._api.sellLimit(symbol, adjustedQuantity, adjustedPrice)
      break
    default:
  ...
  async listOrders({ currency, baseCurrency }) {
    let symbol = `${baseCurrency}-${currency}`
    let orders = await this._api.orders(symbol)
    return orders.map(order => ({
      side: order.OrderType.includes('BUY') ? 'BUY' : 'SELL',
      date: order.TimeStamp,
      quantity: order.Quantity.minus(order.QuantityRemaining).toNumber(),
      price: order.PricePerUnit.toNumber()
    }))
  }
  ...
  async cancelOrder({ orderId }) {
    return this._api.cancel(orderId)
  }
  ...
  async minimumTradeQuantity({ currency, baseCurrency }) {
    ...
    return info.MinTradeSize.toNumber()
  }
