...
class BittrexClient {
  ...
  async openOrder({ currency, baseCurrency, side, quantity, price }) {
    ...
    let pricePrecision = _pricePrecision(price, symbol)
    let adjustedPrice = _parseValue(price, pricePrecision)
...
function _pricePrecision(price, symbol) {
  if (symbol.startsWith('USD') && parseFloat(price) >= 1) {
    return 3
  }
  return 8
}
