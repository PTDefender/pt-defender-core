...
class DefenderCore extends EventEmitter {
   ...
   * @return {Promise}
   */
  async init({ authToken, propertiesPath, session }) {
    ...
    let exchange = properties.get('trading.exchange')
    ...
    this._api = new Api({ authToken, exchange, tradingKey, tradingSecret, session })
