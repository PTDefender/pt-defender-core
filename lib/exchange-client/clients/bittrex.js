...
class BittrexClient {
  ...
  openOrder({ currency, baseCurrency, side, quantity, price }) {
    ...
    switch (side) {
      case 'BUY':
        promise = this._api.marketBuyLimit(symbol, _parseQuantity(quantity), _parseValue(price))
        break
      case 'SELL':
        promise = this._api.marketSellLimit(symbol, _parseQuantity(quantity), _parseValue(price))
        break
      default:
