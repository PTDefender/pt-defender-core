...
class KuCoinClient {
  ...
  constructor(id, { key, secret }) {
    ...
    this._api = new ccxt[id](key, secret)
  }
  ...
  async minimumTradeQuantity({ currency, baseCurrency }) {
    let { limits } = this.marketInfo({ currency, baseCurrency })
  ...
  async marketInfo({ currency, baseCurrency }) {
    await this._api.loadMarkets()
    return this._api.markets[`${baseCurrency}/${currency}`]
  }
}
