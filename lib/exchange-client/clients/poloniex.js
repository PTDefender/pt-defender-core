...
class PoloniexClient {
  ...
  async openOrder({ currency, baseCurrency, side, quantity, price }) {
    ...
    switch (side) {
      case 'BUY':
        result = await this._api.buy(symbol, _parseValue(price), _parseQuantity(quantity))
        break
      case 'SELL':
        result = await this._api.sell(symbol, _parseValue(price), _parseQuantity(quantity))
        break
      default:
...
function _parseQuantity(quantity) {
  let adjusted = quantity - (quantity * .002)
