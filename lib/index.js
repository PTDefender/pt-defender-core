...
class DefenderCore {
  ...
  async init({ authToken, propertiesPath, session, api }) {
    ...
    let serverToken = properties.get('server.api_token').trim()
    ...
    let license = properties.get('license').trim()
    let exchangeType = properties.get('trading.exchange').trim()
    let tradingKey = properties.get('trading_api_key').trim()
    let tradingSecret = properties.get('trading_api_secret').trim()
    let port = properties.get('server.port').trim()
